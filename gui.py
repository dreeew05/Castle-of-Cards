...
def initialize():
    #dictionary that will initialize the default values of the game
    dict = {}
    players = ["Player", "CPU"]
    attrib = ["hp", "dmg"]
    attrib_val = [15, 3]
...
def reset_game_dict():
    #resets certain values when player progresses 
    players = ["Player", "CPU"]
    for player in players:
...
def choose_character_window(prev_window, screenWidth, screenHeight):
    #player will choose a character to play with
...
def configItems(bg, char_bg, char_bg_img, char_img, char_image, rectangle, char_name, char_name_txt, char_description, char_description_txt, char_ability_text, char_ability_icon, char_ability_icon_img, char_ability, char_ability_txt):
    #function that will change information about the character that was clicked
...
def showCharacter1(bg, char_bg, char_img, char_img_width, char_img_height, rectangle, char_name, char_description, char_ability_text, char_ability_icon, icon_width, icon_height, char_ability, lockin_bt):
    #shows detail about the berserker
    global bg1
...
def showCharacter4(bg, char_bg, char_img, char_img_width, char_img_height, rectangle, char_name, char_description, char_ability_text, char_ability_icon, icon_width, icon_height, char_ability, lockin_bt):
    #shows detail about the assassin
    global bg4
...
def play(prev_window, screenWidth, screenHeight, char_name):
    ...
    global greeting_img
    greeting_img = Image.open("img/pixel_art/greetings.png")
...
def floor(prev_window, screenWidth, screenHeight, char_name, wins):
    ...
    #Intro for Final Boss Second Phase
    if wins == 4:
        enemy_intro_txt = bg.create_text(-screenWidth+900, screenHeight/2.75, text="OVERLORD\n(SECOND PHASE)", font=("Gotham Black", 80), fill="white", anchor="nw")
    ...
    player_play = bg.create_image(225, 375, image=player, anchor="nw")
    enemy_play = bg.create_image(screenWidth-530, 375, image=enemy_play_img, anchor="nw")
...
def card_img_map():
    #function that will return certain information about the card
    card_width = 200
    card_height = 300
...
def show_cards(bg, char_name, enemy):
    #show all card images that can be clicked by the user
...
def clickCard(prev_window, frame, player, enemy, player_card, player_card_img):
    #function that will simulate interaction when a card is clicked
    ...
    img_width_combat = 900
    img_height_combat = 900
    ...
    player_combat = bg.create_image(-125, 25, image=player_combat_img, anchor="nw")
    enemy_combat = bg.create_image(1145, 25, image=enemy_combat_img, anchor="nw")
    ...
    dialogue_text = dialogue_canvas.create_text(40, 40, font=("Gotham Black", 47), fill="white", anchor="nw", text="")

    game_dict["turn"] += 1                              #add turns for the enemy (passive purposes)
    ...
    #Both are stunned
    if player_dict["pwr"]["shocked"] > 0 and cpu_dict["pwr"]["shocked"] > 0:
        player_card = "stun"
        enemy_card = "stun"
        dialogue_canvas.itemconfig(dialogue_text, text="You are stunned.")
        updated_texts.extend([enemy["name"].title() + " is stunned."])
    ...
    if player_card == "attack":
        ...
        #Player only deals half DMG if enemy type is a slime
            ...
            updated_texts.extend(["You only deal " + str(player_dict["dmg"]//2) + " DMG because of the Slime's\n passive."])
    ...
    #Player uses Block card interactions
        ...
        #Player reflects Shock card effect with block
            ...
            print("Line 593")
            updated_texts.extend(["Shock reflected! The " + enemy["name"].title() + " will be stunned\n next turn."])
    ...
    #Player uses Cleave card interactions
        ...
        if enemy_card == "block":
            print("Line 655")
            updated_texts.extend(["Special card reflected! You receive " + str(2*player_dict["dmg"]) + " DMG."])
    ...
    if enemy_card == "attack":
        ...
        elif enemy["name"].lower() == "overlord":
            print("Line 722")
            updated_texts.extend([enemy["name"].title() + " deals " + str(cpu_dict["dmg"]) + " DMG and restores " + str(cpu_dict["dmg"]//2) + " HP."])
            player_dict["hp"] -= cpu_dict["dmg"]
            cpu_dict["hp"] += cpu_dict["dmg"]//2
    ...
    if player_win or player_lose:
        if player_win:
            ...
            #Final Boss Second Phase
            if game_dict["wins"] == 4:
                updated_texts.extend(["You have defeated the Overlord, but...",
                "You know how some bosses have a second phase\nwhere they become stronger?",
                "Well... That's exactly what's happening.",
                "The Overlord comes back to life with 8 HP\nand gains +2 DMG!",
                "You have rested for a bit and regained 4 HP."])
                game_dict["Player"]["hp"] += 4
            #Game is Done
            elif game_dict["wins"] == 5:
                updated_texts.extend(["You have defeated the Overlord!"])
            #Minions
            else:
...
def update(widgets, bg, player, enemy):
    ...
    screenWidth = bg.winfo_width()
    screenHeight = bg.winfo_height()
    ...
    if player_win:
        ...
        #Game Continues
            ...
            floor(bg, screenWidth, screenHeight, player, game_dict["wins"])
    elif player_lose:
        #Ending Screen
        close_widget(widgets)
        end_credits(bg)
        ...
        if len(player_hand) == 0 and len(cpu_hand) == 0:
            #refresh both player's and enemy's cards
            ...
            #refresh player's cards only
            player_hand.extend(cards.get_hand(player.lower()))
        elif len(cpu_hand) == 0:
            #refresh enemy's cards only
            cpu_hand.extend(cards.enemy_get_cards(enemy["name"].lower()))
...
def instructions(program_window, icon):
    #function that contains the instructions of the game
    canvas = tk.Canvas(program_window, bg="white")
    canvas.place(relx=0.25, rely=0.1, relwidth=0.5, relheight=0.8)
    instructions = canvas.create_text(400, 210, text="COMBAT\n> The Player will have 15 HP at the start of the playthrough."
    "\n> Common enemies will have 10 HP, while the Final Boss will have 15 HP.\n> The first one to run out of HP loses."
    "\n> Both the player and the opponent will have a DMG value of 3 at start of the battle."
    "\n> Both the player and the opponent chooses a card each turn and plays it at the same time."
    "\n> Each card has its own effect (check Game Encyclopedia to learn about card effects)."
    "\n> A hand contains 6 cards, 5 common cards and 1 unique card."
    "\n> When a hand runs out of cards, it is replenished."
    "\n> There can't be more than 2 copies of any card type in a single hand.\n"
    "\nGAME PROGRESSION"
    "\n> The Castle of Cards will have 4 floors."
    "\n> The First 3 floors will contain a random enemy."
    "\n> You won't be able to fight enemies that have already been encountered in your playthrough."
    "\n> The 4th floor will contain the Final Boss: Overlord."
    "\n> The player restores 8 HP after every encounter, but the player's DMG value resets to 3."
    "\n> You need to clear all 4 floors to win the game.", font=("Arial", 14), fill="black")
...
def show_character_play(char_name):
    #function that will return certain information about the character
    img_location = {
        "berserker": "img/pixel_art/berserker.png",
        "ranger": "img/pixel_art/ranger.png",
        "mage": "img/pixel_art/mage.png",
        "assassin": "img/pixel_art/assassin.png",
        "tank": "img/pixel_art/tank.png",
    }
    return img_location[char_name]

def generate_enemy(index):
    #function that will return certain information about the enemy
    enemies = {
        0: {
            "name": "Slime",
            "img_location": "img/pixel_art/slime_intro.png",
            "bg_location": "img/slime_bg.png",
            "play_location": "img/pixel_art/slime.png",
            "frame_color": "#53a785",
            "box_color": "#0085A9"
        },
        1: {
            "name": "Orc",
            "img_location": "img/pixel_art/orc_intro.png",
            "bg_location": "img/orc_bg.png",
            "play_location": "img/pixel_art/orc.png",
            "frame_color": "#898172",
            "box_color": "#617C63"
        },
        2: {
            "name": "Bandit",
            "img_location": "img/pixel_art/bandit_intro.png",
            "bg_location": "img/bandit_bg.png",
            "play_location": "img/pixel_art/bandit.png",
            "frame_color": "#8a2800",
            "box_color": "#B961A8"
        },
        3: {
            "name": "Undead",
            "img_location": "img/pixel_art/undead_intro.png",
            "bg_location": "img/undead_bg.png",
            "play_location": "img/pixel_art/undead.png",
            "frame_color": "#848476",
            "box_color": "#5E7C67"
        },
        4: {
            "name": "Overlord",
            "img_location": "img/pixel_art/overlord_1_intro.png",
            "bg_location": "img/overlord_bg.png",
            "play_location": "img/pixel_art/overlord_1.png",
            "frame_color": "#a0384e",
            "box_color": "#6A861E"
        },
        5: {
            "name": "Overlord",
            "img_location": "img/pixel_art/overlord_2_intro.png",
            "bg_location": "img/overlord_bg.png",
            "play_location": "img/pixel_art/overlord_2.png",
            "frame_color": "#a0384e",
            "box_color": "#6A861E"
        }
    }
...
def close_widget(widgets):
    #receives all the widget from a list and destroy each of it
    for widget in widgets:
        widget.destroy()

def end_credits(prev_window):
    #ending screen
    screenWidth = prev_window.winfo_width()
    screenHeight = prev_window.winfo_height()
    bg = tk.Canvas(prev_window, width=screenWidth, height=screenHeight, bg="grey", highlightthickness=0)
    ...
    msg = bg.create_text(screenWidth/2, screenHeight/2, text="", font=("Gotham Black", 70), fill="white", anchor="center")

    if game_dict["wins"] == 5:
        print("Congratulations, You have cleared Castle of Cards!")
        bg.itemconfig(msg, text = "Congratulations,\nYou have cleared Castle of Cards!")
    else:
        bg.itemconfig(msg, text = "Better luck next time!")

    button = tk.Button(bg, text="QUIT", font=("Gotham Black", 15), width=15, padx=10, pady=10, command=quit)
    button.place(relx=0.5, rely=0.75, anchor="center")

def remove_status(status, status_arr):
    #function that removes status for both player and cpu
    while status in status_arr:
        status_arr.remove(status)

def change_text(msgs, cnv, txt, widgets, bg, player, enemy):
    #this function will change the dialogue text
    print("Enemy:", enemy)
    print(game_dict)
    print(msgs)
    if len(msgs) == 0:
        #when there is no message left, proceed to update()
        update(widgets, bg, player, enemy)                     
    else:
        #change text whenever there is a message left
